...
# KPI card — NO icon parameter to avoid HTML rendering issue
def kpi_card(label, value, delta=None, accent="#A5B4FC"):
    delta_html = ""
...
with st.sidebar:
    ...
    if data_mode == "Upload Custom Dataset":
        ...
        try:
            daily_data, store_list, _ = process_uploaded(open('train.csv', 'rb').read())
            st.success("✅ Demo data loaded (Rossmann Store Sales)")
        except:
            st.error("❌ train.csv not found in project folder")
    ...
    if forecast_days <= 30:
        model_opts = ["XGBoost", "Prophet", "ARIMA"]
        rec_note   = "XGBoost — best for short-term"
    else:
        model_opts = ["Prophet", "XGBoost", "ARIMA"]
        rec_note   = "Prophet — best for long-term"

    st.caption(f"💡 Recommended: **{rec_note}**")

    model_choice = st.selectbox("🤖 Select Model", model_opts, index=0)

    if model_choice == "XGBoost" and forecast_days >= 60:
        st.warning("⚠️ XGBoost accuracy drops for 60+ days. Prophet recommended.")
    ...
    st.markdown("---")
    st.button("🚀 Generate Forecast", use_container_width=True, type="primary")
...
# ─────────────────────────────────────────
# KPI CARDS — fixed growth calculation
# ─────────────────────────────────────────
...
# Growth = daily avg comparison (same unit)
avg_last30    = float(df_filtered['Sales'].tail(30).mean())
avg_forecast  = float(np.mean(fut_pred))
growth        = ((avg_forecast - avg_last30) / avg_last30 * 100) if avg_last30 else 0
...
with k1:
    st.markdown(kpi_card(
        ...
        format_inr(total_hist),
        accent="#A5B4FC"
    ), unsafe_allow_html=True)
with k2:
    st.markdown(kpi_card(
        f"Predicted Next {forecast_days}d",
        format_inr(total_fut),
        delta=growth,
        accent="#34D399"
    ), unsafe_allow_html=True)
with k3:
...
with tab1:
    ...
    with col_b:
        bmap = {"ARIMA":"badge-blue","Prophet":"badge-purple","XGBoost":"badge-green"}
        st.markdown(
            f'<span class="badge {bmap.get(model_choice,"badge-blue")}">{model_choice}</span>',
            unsafe_allow_html=True
        )
    ...
    # Today line using shape (no add_vline — avoids timestamp error)
    ...
    df_tbl = pd.DataFrame({
        ...
        'vs Daily Avg'   : [
            f"{'▲' if v > avg_daily else '▼'} {abs(v-avg_daily)/avg_daily*100:.1f}%"
            for v in fut_pred
        ],
    })
